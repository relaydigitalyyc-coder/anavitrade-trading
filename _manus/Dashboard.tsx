...
export default function Dashboard() {
  ...
  const apiWallet = liveData?.wallet;

  const { data: web3Session, refetch: refetchWeb3 } = trpc.web3Wallet.getSession.useQuery();

  const apiConnected = apiWallet?.status === "active";
  const web3Connected = web3Session?.status === "active";
  const anyConnected = apiConnected || web3Connected;

  const statusLabel = apiConnected ? "Live Trading"
    : web3Connected ? "Wallet Connected"
    : apiWallet?.status === "pending" ? "Wallet Pending"
    : "Not Connected";

  const statusColor = apiConnected || web3Connected
    ? "bg-primary/10 border-primary/20 text-primary"
    : apiWallet?.status === "pending"
      ? "bg-amber-400/10 border-amber-400/20 text-amber-400"
      : "bg-border/50 border-border text-muted-foreground";

  const dotColor = apiConnected || web3Connected ? "bg-primary animate-pulse"
    : apiWallet?.status === "pending" ? "bg-amber-400"
    : "bg-muted-foreground";
  ...
  const handleKillSwitch = () => {
    ...
    else if (apiConnected) toggleKill.mutate({ active: !killActive });
  };
            ...
            {apiWallet?.status === "pending" ? (
              <div className="p-5 rounded-2xl border bg-amber-400/5 border-amber-400/20">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <h3 className="text-sm font-semibold text-foreground mb-1">API Wallet Pending Validation</h3>
                    <p className="text-xs text-muted-foreground">Your API wallet is connected and awaiting validation.</p>
                  </div>
                </div>
              </div>
            ) : (
              <div className="p-5 rounded-2xl border bg-primary/5 border-primary/20">
                <div className="flex items-start justify-between gap-4 flex-wrap">
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      <h3 className="text-sm font-semibold text-foreground mb-1">Connect Your Wallet to Start Copy-Trading</h3>
                      <p className="text-xs text-muted-foreground max-w-lg">
                        Choose between a Ledger / Web3 wallet or a Hyperliquid API wallet. Your funds stay in your own account.
                      </p>
                    </div>
                  </div>
                  <div className="flex items-center gap-2 shrink-0 flex-wrap">
                    ...
                    <Link href="/onboarding/hyperliquid">
                      <button className="flex items-center gap-2 px-4 py-2 rounded-xl border border-border text-foreground text-sm font-medium hover:bg-card transition-all">
                        <Wifi className="w-3.5 h-3.5" /> API Wallet
                      </button>
                    </Link>
                  </div>
                </div>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
          {[
            ...
            },
            {
              ...
              sub: anyConnected
                ? killActive ? "Kill switch on" : web3Connected ? "Ledger/Web3 ready" : "API signals running"
                : "No wallet connected",
              azure: false,
              gold: false,
            },
          ...
          <div className="p-6 border-b border-border/40">
            <div className="flex items-center justify-between mb-4">
              <div>
                ...
                <p className="text-xs text-muted-foreground mt-0.5">
                  {signalsTotal > 0 ? `${signalsTotal.toLocaleString()} signals · Binance USDT · Updated every 5 min` : "Loading signals..."}
                </p>
              </div>
              <div className="flex items-center gap-2">
                <button
