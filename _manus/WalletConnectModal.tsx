...
export default function WalletConnectModal({ isOpen, onClose, onConnected }: WalletConnectModalProps) {
  ...
  }, [isConnected, wagmiAddress, step, hasSaved, chainId, selectedWallet, persistWallet]);
  ...
  const initiateWagmiConnect = useCallback((wallet: WalletOption) => {
    setStep("connecting");
    const connector = connectors.find(c => {
      const cid = c.id.toLowerCase();
      const wid = wallet.connectorId.toLowerCase();
      return cid.includes(wid) || wid.includes(cid) || cid === wid;
    });
    if (!connector) {
      if (wallet.id === "metamask") {
        setErrorMsg("MetaMask not detected. Please install the MetaMask extension from metamask.io and refresh the page.");
      } else {
        setErrorMsg(`${wallet.name} connector not available. Please ensure the wallet is installed and try again.`);
      }
      setStep("error");
      return;
    }
        ...
        >
          <motion.div
            ...
            style={{
              ...
              boxShadow: "0 25px 80px rgba(0,0,0,0.8), 0 0 0 1px rgba(74,222,128,0.05)",
            }}
          >
            {/* Header */}
              ...
              <AnimatePresence mode="wait">
                ...
                {step === "select" && (
                  <motion.div key="select" initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 10 }}>
                    ...
                    <div className="rounded-xl p-4" style={{ background: "rgba(74,222,128,0.04)", border: "1px solid rgba(74,222,128,0.08)" }}>
                      <p className="text-emerald-400 text-xs font-semibold uppercase tracking-wider mb-3">Security Guarantees</p>
                      <div className="grid grid-cols-2 gap-2">
                        {securityGuarantees.map((g, i) => (
                          <div key={i} className="flex items-start gap-2">
                            <span className="text-emerald-400 mt-0.5 flex-shrink-0">{g.icon}</span>
                ...
                {step === "ledger-guide" && (
                  <motion.div key="ledger" initial={{ opacity: 0, x: 10 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -10 }}>
                        ...
                        <div className="flex items-center gap-3 mb-2">
                          ...
                          <div className="flex-1">
                            <div className="flex items-center gap-2">
                              ...
                              <span className="text-[10px] font-semibold px-1.5 py-0.5 rounded-full border text-emerald-400 bg-emerald-400/10 border-emerald-400/20">Recommended</span>
                            </div>
                        ...
                        <div className="ml-11 space-y-1">
                          {["Plug your Ledger Nano X or S Plus into USB", "Unlock with your PIN", "Open the Ethereum app on your Ledger"].map((s, i) => (
                            <div key={i} className="flex items-center gap-2">
                              <span className="w-4 h-4 rounded-full flex items-center justify-center text-[9px] font-bold text-emerald-400" style={{ background: "rgba(74,222,128,0.1)" }}>{i + 1}</span>
                    ...
                    <div className="p-3 rounded-xl mb-5 flex items-start gap-2" style={{ background: "rgba(74,222,128,0.05)", border: "1px solid rgba(74,222,128,0.1)" }}>
                ...
                {step === "ledger-connecting-direct" && (
                  <motion.div key="ledger-direct" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
                    ...
                    <div className="relative mb-6">
                      ...
                      <div className="absolute -bottom-1 -right-1 w-6 h-6 rounded-full bg-[#0a0f1a] border border-white/10 flex items-center justify-center">
                    ...
                    <div className="mt-3 p-3 rounded-xl w-full text-left" style={{ background: "rgba(74,222,128,0.04)", border: "1px solid rgba(74,222,128,0.08)" }}>
                      {["Ledger is plugged in and unlocked", "Ethereum app is open on the device", "Browser permission dialog approved"].map((s, i) => (
                        <div key={i} className="flex items-center gap-2 py-1">
                          <div className="w-4 h-4 rounded-full flex items-center justify-center" style={{ background: "rgba(74,222,128,0.1)" }}>
                ...
                {(step === "connecting" || step === "saving") && (
                  <motion.div key="connecting" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
                    ...
                    {step === "connecting" && selectedWallet?.id === "coinbase" && (
                      <p className="text-white/50 text-sm max-w-xs">Check Coinbase Wallet and approve the connection request.</p>
                    )}
                    {step === "saving" && (
                  ...
                  <motion.div key="error" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
                    ...
                    {errorMsg?.toLowerCase().includes("metamask") && (
                      <a
                        ...
                        className="inline-flex items-center gap-1.5 text-emerald-400 text-sm mb-4 hover:underline"
                      >
