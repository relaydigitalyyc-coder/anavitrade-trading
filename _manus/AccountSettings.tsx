...
export default function AccountSettings() {
  ...
  const tabs = [
    ...
    { id: "wallet" as Tab, label: "API Wallet", icon: <Key className="w-4 h-4" /> },
...
function WalletTab() {
  ...
  const { data: walletData, isLoading, refetch } = trpc.apiWallet.getStatus.useQuery();

  const revoke = trpc.apiWallet.revoke.useMutation({
    onSuccess: () => {
      toast.success("API wallet access revoked. Trade execution stopped.");
      setConfirmRevoke(false);
      refetch();
    },
    onError: () => toast.error("Failed to revoke wallet access."),
  });
  ...
  return (
    <div className="space-y-6">
      <SectionHeader title="API Wallet" subtitle="Manage your Hyperliquid API wallet connection. You can revoke access at any time." />
      ...
      {isLoading ? (
        ...
        <div className="p-6 rounded-2xl bg-card border border-border/50 space-y-5">
          <div className="flex items-start justify-between">
            <div>
              <div className="flex items-center gap-2 mb-1">
                <span className={`text-xs font-semibold px-2.5 py-1 rounded-full border ${statusColors[walletData.status] ?? statusColors.pending}`}>
                  {walletData.status.toUpperCase()}
                </span>
                {walletData.isLedgerCustody && (
                  <span className="text-xs font-semibold px-2.5 py-1 rounded-full border border-primary/20 bg-primary/5 text-primary flex items-center gap-1">
                    <HardDrive className="w-3 h-3" /> Ledger
                  </span>
                )}
              </div>
              <p className="text-xs text-muted-foreground mt-2">Connected {new Date(walletData.createdAt).toLocaleDateString()}</p>
            </div>
          ...
          <div className="space-y-3">
            <InfoRow label="Hyperliquid Account" value={walletData.hyperliquidAccount} mono />
            <InfoRow label="API Wallet Address" value={walletData.walletAddress} mono />
            <InfoRow label="Permissions" value="Trade execution only — no withdrawal access" />
            {walletData.validatedAt && <InfoRow label="Validated" value={new Date(walletData.validatedAt).toLocaleString()} />}
          </div>
          ...
          {walletData.status === "active" && (
            <div className="pt-4 border-t border-border/50">
              {!confirmRevoke ? (
                ...
                >
                  <ZapOff className="w-4 h-4" /> Revoke API Wallet Access
                </button>
              ) : (
                <div className="p-4 rounded-xl bg-red-500/5 border border-red-500/20">
                  <p className="text-sm text-foreground font-semibold mb-1">Confirm revocation?</p>
                  <p className="text-xs text-muted-foreground mb-4">This will immediately stop all trade execution on your account. You can reconnect a new API wallet at any time.</p>
                  <div className="flex gap-3">
                    ...
                    >
                      {revoke.isPending ? "Revoking..." : "Yes, Revoke Access"}
                    </button>
          ...
          {walletData.status === "revoked" && (
            <div className="pt-4 border-t border-border/50">
              <p className="text-sm text-muted-foreground mb-3">Access was revoked on {walletData.revokedAt ? new Date(walletData.revokedAt).toLocaleString() : "unknown date"}.</p>
              <button
                onClick={() => navigate("/onboarding/hyperliquid")}
              ...
              >
                <Key className="w-4 h-4" /> Reconnect API Wallet
              </button>
            </div>
          )}
        </div>
        ...
        <div className="p-6 rounded-2xl bg-card border border-border/50 text-center">
          ...
          <h3 className="text-sm font-semibold text-foreground mb-2">No API wallet connected</h3>
          <p className="text-xs text-muted-foreground mb-5">Connect your Hyperliquid API wallet to start live trading.</p>
          <button
            onClick={() => navigate("/onboarding/hyperliquid")}
          ...
          >
            <Key className="w-4 h-4" /> Connect API Wallet
          </button>
        </div>
      )}
      ...
      <div className="p-4 rounded-xl bg-card border border-border/50">
        <h4 className="text-sm font-semibold text-foreground mb-2 flex items-center gap-2">
          <ExternalLink className="w-4 h-4 text-primary" /> Revoke directly on Hyperliquid
        </h4>
        <p className="text-xs text-muted-foreground mb-3">
          You can also revoke the API wallet directly from your Hyperliquid account at any time, without needing to log in here.
        </p>
        <a
        ...
        >
          Open Hyperliquid Settings <ExternalLink className="w-3 h-3" />
        </a>
      </div>
    </div>
  );
